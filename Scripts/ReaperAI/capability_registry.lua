...
function CapabilityRegistry.create(deps)
  ...
  local CAPABILITY_CARDS = {
    ...
    ["region/set_color"] = { intent = "set Region color only", target_binding = "region id/range/name/all/selected/current/time_selection", params = { "index", "id", "region", "target", "range", "ids", "start", "end", "order_start", "order_end", "name", "match", "selected", "scope", "all", "color" }, prefer_mcp_when = "Region color changes, including inferred selected Region", allow_script = false, risk = "modify", verifier = "region/property", repair = "use Region-specific target and color; never route through Marker operations" },
    ["item/set_color"] = { intent = "set MediaItem color only", target_binding = "index/name/selected/all/time_selection/created.items", params = { "index", "name", "match", "target", "item", "selected", "all", "time_selection", "scope", "color" }, prefer_mcp_when = "Item color changes", allow_script = false, risk = "modify", verifier = "item/property", repair = "use color name, #RRGGBB, r,g,b, or color=default/clear with explicit item target" },
