...
**Region 选择说明**：
- REAPER 原生 API 不稳定暴露“Region 管理器选中状态”；ReaperAI 会把当前时间选区精确匹配、完全包含或重叠的 Region 作为“推断选中 Region”注入上下文。
- 用户说“选中的 Region / 选中的区域 / selected region”时，输出 ACTION_PLAN：`region.delete scope=selected`，由通用 selector 编译器解析实时候选。
- 用户说“除了/保留选中的 Region，其他 Region 全部删除”时，输出 ACTION_PLAN：`region.delete scope=all exclude=selected`。
- 不要使用 `region/delete?selected=true` 或 `region/delete?all=true&keep_selected=true`；Region 的上下文选择和排除都走通用 selector。
...
- track/set_color - 设置轨道颜色，支持 name/index/range/ids/order_start/order_end/selected/all 和 color=红色/#FF0000/r,g,b
- track/clear_color - 清除自定义轨道颜色/恢复默认主题颜色，支持 name/index/selected/all
- item/set_color - 设置 MediaItem 颜色，支持 index/range/ids/order_start/order_end/name/selected/all/time_selection 和 color=黄色/#FFFF00/r,g,b；只作用于 Item
- region/set_color - 设置 Region 颜色，支持 index/range/ids/order_start/order_end/name/all/selected/current/time_selection 和 color=黄色/#FFFF00/r,g,b；只作用于 Region，永远不要用于 Marker
...
- 调用 `reaper.SetProjectMarker3` 必须传 7 个参数：`proj, markrgnindexnumber, isrgn, pos, rgnend, name, color`；Region 改色时 `isrgn` 必须是 `true`，颜色必须是 `reaper.ColorToNative(r,g,b) + 16777216`；如果没有颜色值，最后一个参数传 `0`。
...
function Prompt.append_stable_script_prompt(sys)
  ...
  sys = sys .. "10. 调用 reaper.SetProjectMarker3 必须传 7 个参数；Region 改色时 isrgn=true，color 必须用 reaper.ColorToNative(r,g,b)+16777216，最后的 color 不知道时传 0。\n"
  ...
  sys = sys .. "Region delete rule: selected/current/time-selection Regions use ACTION_PLAN region.delete scope=selected/current/time_selection. Keep selected Region and delete the rest uses region.delete scope=all exclude=selected. Explicit R ids or ID wording use region/delete start/end or index=N. Ambiguous bare numeric Region ranges must clarify R ids vs timeline order. Never route Region deletion to marker/delete.\n"
  sys = sys .. "Color edit rule: track color uses track/set_color, item color uses item/set_color, and Region color uses region/set_color. Region and Marker are different objects; never use Marker operations for Region color. Prefer MCP/ACTION_PLAN over SCRIPT for all three color operations.\n"
  return sys
end
...
function Prompt.append_intent_contract_prompt(sys)
  ...
  sys = sys .. "Generic selector override: region/delete no longer accepts selected=true, time_selection=true, keep_selected=true, except_selected, preserve_selected, or exclude_selected. Use ACTION_PLAN scope/exclude selectors for contextual or exception-based Region/Marker operations.\n"
  sys = sys .. "For region/delete MCP calls, require an explicit Region id/range/name target or all=true. For contextual Region targets, output ACTION_PLAN with region.delete scope=selected/current/time_selection. For keep/except selected and delete the rest, output region.delete scope=all exclude=selected. R15-R20 or explicit ID wording means start=15&end=20. Bare numeric Region ranges are ambiguous between R ids and timeline order and must clarify. Do not use marker/delete for Regions.\n"
  sys = sys .. "Final selector override: never output region/delete?selected=true or region/delete?all=true&keep_selected=true. Use region.delete scope=selected/current/time_selection, or region.delete scope=all exclude=selected, and let the app compile a generic selector script.\n"
...
function Prompt.append_action_protocol_prompt(sys)
  ...
  sys = sys .. "Preferred action names include: transport.play, transport.stop, track.create, track.delete, track.rename, track.volume.set, track.pan.set, track.color.set, track.color.clear, track.mute, track.solo, track.fx.add, track.fx.remove, track.folder.group, marker.add, marker.delete, region.delete, region.color.set, region.rename.batch, item.delete, item.color.set, item.fade, item.fade_shape, envelope.draw, envelope.clear, sfx.generate_variants, analysis.detect_peaks, analysis.find_loop_points, export.regions, export.tracks, export.master, native.action.\n"
  sys = sys .. "Selectors are normalized by the app. Use all=true for natively supported all-object endpoints, selected=true for endpoints that natively support selected tracks/items, and scope=all|selected|current|time_selection plus exclude=selected|current|time_selection when a range or exception must be compiled generically. Use index=N for one object, name=text for named targets, match=text for substring batches, ids=1,2,3 for explicit ids, start/end for Region displayed ids, and order_start/order_end for timeline-order Region ranges.\n"
  ...
  sys = sys .. "For color changes, use the object-specific endpoint/action: track.color.set for tracks, item.color.set for MediaItems, region.color.set for Regions. Do not use SCRIPT for Region or Item color when these endpoints can express the request.\n"
  ...
  sys = sys .. "For deleting Regions, use region.delete with ids=, start/end, order_start/order_end, name=, match=, or all=true for explicit MCP targets. Use region.delete scope=selected/current/time_selection for contextual Region targets, and region.delete scope=all exclude=selected to preserve contextual selection while deleting the rest. Never route Region deletion through marker.delete.\n"
