...
set "LOG_FILE=%TEMP%\ReaperAI_config_wizard_last_error.log"
set "PYTHON_EXE="
set "PYTHONW_EXE="
set "BASE_PYTHON="
...
if errorlevel 1 (
  ...
  echo Run Step 1 first, or keep python-%PYTHON_VERSION%-embed-amd64.zip under:
  echo   %INSTALLER_DIR%
  echo.
...
"%PYTHON_EXE%" "%HELPER%" wizard "%SCRIPT_ROOT%"
set "WIZARD_EXIT=%ERRORLEVEL%"
echo.
if not "%WIZARD_EXIT%"=="0" (
  echo [ERROR] Config wizard failed. Exit code: %WIZARD_EXIT%
  if exist "%LOG_FILE%" echo Detail log: %LOG_FILE%
  pause
  exit /b %WIZARD_EXIT%
)
