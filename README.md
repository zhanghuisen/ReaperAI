...
- 重构执行意图协议：移除僵硬的澄清/阻断循环，让 AI 能基于上下文继续对话、纠错和生成操作计划。
- 增强 MCP 端点优先能力：补齐常用批量操作，支持批量删除 Marker/Region、批量轨道属性修改等高频工作流。
- 修复轨道默认颜色：清除自定义轨道颜色现在恢复 REAPER 主题默认色，不再错误显示为黑色。
- 改进失败修复：保留失败后的修复能力，但改为基于上次错误证据分析，不再简单重复同一套输出。
- 修复窗口折叠/展开显示问题：隐藏后移动窗口再展开时，会自动恢复到完整可见尺寸。
- 第二步配置向导中文化：BAT 只做稳定启动器，中文交互交给 UTF-8 Python 层处理，减少闪退、乱码和路径输入问题。
